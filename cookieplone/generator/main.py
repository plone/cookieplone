...
from cookieplone.utils import rmtree
from cookieplone.utils.answers import remove_internal_keys, write_answers
...
def cookieplone(
    ...
    config_file: os.PathLike[str] | str | None = None,
    ...
    :param keep_project_on_failure: If `True` keep generated project directory even when
        generation fails
    :param dump_answers: If `True` dump provided answers to a json file.
    """
    ...
    context["_cookiecutter"] = remove_internal_keys(context["cookiecutter"])

    # prompt the user to manually configure at the command line.
    # except when 'no-input' flag is set
    with import_patch:
        if context_for_prompting["cookiecutter"]:
            context["cookiecutter"].update(wizard(context_for_prompting, no_input))

    logger.debug(f"context is {context}")

    # Dump answers to a json file. This is useful for debugging and reruns
    dump_answers_path = None
    if dump_answers:
        dump_answers_path = write_answers(context["cookiecutter"], template_name)
        logger.debug(f"Dumped answers to {dump_answers_path}")
    ...
    # Move dumped_answers inside created codebase
    if dump_answers_path:
        dump_answers_path.rename(Path(result) / COOKIEPLONE_ANSWERS_FILE)
    return result
